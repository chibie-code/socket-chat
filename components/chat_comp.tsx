...
import useFetch from './custom-hooks/session-hook';
import io from 'socket.io-client';

function ChatRoomComponent() {

  const [ msg, setMsg ] = React.useState({hello:''});
  const [ userMessage, setUserMessage ] = React.useState({text:''});
  ...
  const handleTypeMessage = (evt) => {
    setUserMessage(prev=>({text: evt.target.value}));
    // console.log("text:",evt.target.value);
    io().emit('send_message', userMessage);
  };
    ...
    return (
      <>
        ...
        <div style={styles.inputArea}>
          ...
          onClick={handleSendMessage}>Send (Enter)</button>
        </div>
      </>
    )
  }
